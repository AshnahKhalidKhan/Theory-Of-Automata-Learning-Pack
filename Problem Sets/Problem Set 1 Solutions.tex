...
\begin{tikzpicture}
  ...
  \path[-stealth]
    ...
    (q0) edge[loop above] node[right] {\(\mathtt b\)} (q0)
    (q0) edge node[above] {\(\mathtt a\)} (qa)
    (qa) edge[loop above] node[right] {\(\mathtt b\)} (qa)
    (qa) edge node[above] {\(\mathtt a\)} (qaa)
    (qaa) edge[loop above] node[right] {\(\mathtt b\)} (qaa)
    (qaa) edge node[above] {\(\mathtt a\)} (qaaa)
    (qaaa) edge[loop above] node[right] {\(\mathtt b\)} (qaaa)
    (qaa) edge node[above] {\(\mathtt a\)} (qaaa)
