...
\begin{enumerate}
    ...
    \item
        ...
        \textbf{Solution:}
        \\Let \(w = xyz\) and \(w = ab^pc^p\). If \(x = \lambda, y = ab^m\) and \(z = b^{p-m}c^p\) then \(w_i = \lambda(a^m)^i(b^{p-m}c^p)\). Pump up to get \(w_2 = (ab^m)^2(b^{p-m}c^p) = ab^mab^mb^{p-m}c^p = ab^mab^pc^p\) which does noy belong to the language so we have a contradiction.
